...
const { GoldPaymentInvoice, Shop } = require("../model");
...
const createInvoice = async (req, res) => {
  ...
  try {
    ...
    const invoice = await GoldPaymentInvoice({
      customerId,
      items,
      total,
      shop,
...
const getInvoices = async (req, res) => {
  ...
  try {
    ...
    const invoices = await GoldPaymentInvoice.find(query)
...
const getInvoiceById = async (req, res) => {
  ...
  try {
    ...
    const invoice = await GoldPaymentInvoice.findOne(query)
      .populate("customerId", "name phone address civilId")
      .populate("shop", "name")
      .populate("createdBy", "name");
...
const getInvoicesByFilter = async (req, res) => {
  try {
    ...
    // ✅ Total count
    const total = await GoldPaymentInvoice.countDocuments(query);
...
const updateInvoice = async (req, res) => {
  ...
  try {
    ...
    const invoice = await GoldPaymentInvoice.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
const deleteInvoice = async (req, res) => {
  ...
  try {
    ...
    await GoldPaymentInvoice.findByIdAndUpdate(
      req.params.id,
      { status: "DELETED" }
    );
