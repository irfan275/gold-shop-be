...
const { ReceiptInvoice } = require("../model");
...
const createInvoice = async (req, res) => {
  ...
  try {
    ...
    const { customerId, items, finalTotal,shop ,notes,invoiceDate} = req.body;
    ...
    const invoice = await ReceiptInvoice({
      customerId,
      items,
      shop,
      notes,
...
const getInvoices = async (req, res) => {
  ...
  try {
    ...
    const invoices = await ReceiptInvoice.find(query)
...
const getInvoiceById = async (req, res) => {
  ...
  try {
    ...
    const invoice = await ReceiptInvoice.findOne(query)
      .populate("customerId", "name phone address civilId")
      .populate("shop", "name")
      .populate("createdBy", "name");
...
const getInvoicesByFilter = async (req, res) => {
  try {
    ...
    // ✅ Total count
    const total = await ReceiptInvoice.countDocuments(query);
...
const updateInvoice = async (req, res) => {
  ...
  try {
    ...
    const invoice = await ReceiptInvoice.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
const deleteInvoice = async (req, res) => {
  ...
  try {
    ...
    await ReceiptInvoice.findByIdAndUpdate(
      req.params.id,
      { status: "DELETED" }
    );
